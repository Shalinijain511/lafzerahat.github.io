...
class wpuserTabSetting
{
    ...
    static function my_profile_tab($atts = array())
    {
        ...
        $wpuser_tab['tab_visible_role'] = array(
            'is_required' => 0,
            'description' => __('Choose the user role that user can see this tab. Leave blank for visible to all user role.', 'wpuser'),
            'type'=>'multiplecheckbox',
            'label' => 'Tab Visible to',
            'options' => $options
        );
                ...
                if (isset($value['type']) && $value['type'] == 'textarea') {
                   echo '<textarea  id="' . $key . '" name="wpuser_tab[' . $key . ']" cols="100" rows="4"></textarea>';
                    ...
                    foreach ($options as $optionKey => $optionValue) {
                        ...
                        echo '<input id="' . $optionKey . '"  type="checkbox" name="wpuser_tab[' . $key . '][]" value="' . esc_attr(strtolower($optionValue)) . '">' . $optionValue ;
                        echo '</div>';
                    }
                    echo '</div>';
                }
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 'success') {
                            if (action == 'delete') {
                                ...
                                if (!(typeof response.data.tab_visible_role === 'undefined')) {
